...
class Infologemgwave : AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_infologemgwave)
...
//        }
    ...
    private fun gyroValues() {
        ...
        val emgQuery = database.child("Emg")
        emgQuery.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                for (itemSnapShot in listSnapshot) {
                    var readKeyGet = itemSnapShot.child("time").getValue().toString();
                    var readKey = readKeyGet.split(":")[0].toFloat()*60.0f*60.0f+readKeyGet.split(":")[1].toFloat()*60.0f+readKeyGet.split(":")[2].toFloat();
                    var readVal =  itemSnapShot.child("emgW").getValue().toString().toFloat()*1.0f;
                    var readEmgVal =  itemSnapShot.child("emg").getValue().toString().toFloat()*1.0f;
                    listValue.add(Data(valueTime = readKey.toLong(), valueData = readVal, valueType = ""))
                    listTime.add(readKey.toLong())
                    listEmg.add(readEmgVal)
                }
